...
public class BuildingScriptBase
{
    ...
    private static void Kill(string exe)
    {
        try
        {
            ExeRunner.Execute(new int[] {128}, "taskkill", "/im", exe, "/f");
        }
    ...
    public string[] A05_IlMerge()
    {
        var start = DateTime.Now;

        var ilmc = Cfg.IlMerge;
        var options = new IlMergeOptions
        {
            InputExe           = Cfg.ExeName,
            Exclude            = ilmc.Exclude?.ToHashSet() ?? new HashSet<string>(),
            OutputExe          = ilmc.OutputExe,
            LogFileName        = ilmc.LogFileName,
            Flags              = ilmc.Flags,
            InternalizeExclude = ilmc.InternalizeExclude,
            Target             = ilmc.Target,
            KeyFile            = @"c:\programs\narzedzia\sign\iSukcesCodeSign.snk",
            // TargetPlatform = @"v4, C:\Windows\Microsoft.NET\Framework\v4.0.30319",
#if USE_ILREPACK
                IlmergeExe = SlnDir("..", "tools", "ILRepack.exe")
#else
            IlmergeExe = SlnDir(ilmc.IlMergeExe)
#endif
        };
        if (string.IsNullOrEmpty(options.OutputExe))
            options.OutputExe = options.InputExe;
        var list = options.AddIlMergedBinaries(options.InputExe, CompiledBinariesDir);

        options.Run(); //ExeName);
        ExConsole.WriteLine("Merge time {0}", DateTime.Now - start);
        return list;
    }
